            ...
            return(
                <div key={data.id+"bannerHome"+index} className='min-w-full min-h-[450px] lg:min-h-full overflow-hidden relative group transition-all' style={{transform :`translateX(-${currentImage * 100}%)`}}>
                     ...
                     <div className=' w-full absolute bottom-0 max-w-md px-3 '>
                      ...
                      <p className='text-ellipse line-clamp-3 my-4'>{data.overview}</p>
                      <div className='flex items-center gap-4'>
                        <p>Rating : {Number(data.vote_average).toFixed(1 )}</p>
                        <span>|</span>
                        <p>View : {Number(data.popularity).toFixed(0)}+ </p>
                    </div>
                    <button className="bg-white px-4 py-2 text-black font-bold rounded mt-4 hover:bg-gradient-to-r from-red-700 to-orange-500 transition-all duration-300 hover:scale-105">
                       Play Now
                    </button>
                    </div>
                    </div>
...
const BannerHome = () => {
  ...
  // ✅ Fixed useEffect (no dependency warning)
  useEffect(() => {
            ...
            return (
              ...
              >
                <div className='w-full h-full'>
                  <img
                    src={imageURL + data.backdrop_path}
                    alt={data?.title || data?.name || "banner"}  // ✅ FIXED
                    className='h-full w-full object-cover'
                  />
                </div>
                ...
                <div className='container mx-auto'>
                  <div className='w-full absolute bottom-0 max-w-md px-3'>
                    ...
                    <p className='text-ellipse line-clamp-3 my-4'>
                      {data.overview}
                    </p>
